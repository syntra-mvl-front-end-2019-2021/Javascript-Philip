...
let valid;
function genBracketPairs() {
    const bracketPairs = function firstIndex(result,val,index,{ valid: true, pairs: [] }) {
        valid = true;
        let pairs = [];
        let code = [];
        for (let i = 0; i <= code.length;i++) {
            if(result.valid) {
                ...
                if (val === ']') {
                    valid = false;
                    pairs = [];
                   result = function secondIndex(pairResult, pair,{ valid: true, pairs: [] }) {
    ...
    ///////////////////////////// !!!!!!!!!!!!!!!!!!!
    let sob = function singleOpenBr() {
        for( let i = 0; i <= bracketPairs.length; i++) {
            return pair.length === 1;
        }
    }
    const singleOpenBracket = bracketPairs.pairs
    bracketPairs.valid = bracketPairs.valid && singleOpenBracket.length === 0;
